...
def main():
    ...
    running = os.path.exists(lockfile)
    ...
    if duration != "none":
        ...
        for zoneUp in duration:
            print "Zone {zone} for {minutes} minutes".format(zone=zoneUp,
                minutes=int(duration[zoneUp]) * 60)

            setZone(int(zoneUp), 'ON')
            time.sleep(int(duration[zoneUp]) * 60)
            setZone(int(zoneUp), 'OFF')
    print "Turning off"
    os.remove(lockfile)
...
def checkSchedule():
    ...
    try:
        scheduleFile = open('/home/pi/sprinklerSchedule.json')
    except:
